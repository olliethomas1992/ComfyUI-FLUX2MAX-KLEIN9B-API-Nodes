...
class TestFlux2MaxImageSlots:
    ...
    def test_no_images_sends_no_input_image_keys(self, mock_post):
        ...
        run_async(Flux2MaxDirect.execute(prompt="test", safety_tolerance=2, output_format="jpeg"))
    ...
    def test_single_image_maps_to_input_image(self, mock_post):
        ...
        run_async(Flux2MaxDirect.execute(prompt="test", safety_tolerance=2, output_format="jpeg", image_1=img))
        args = mock_post.call_args[0][1]
    ...
    def test_multiple_images_map_correctly(self, mock_post):
        ...
        run_async(Flux2MaxDirect.execute(prompt="test", safety_tolerance=2, output_format="jpeg", **imgs))
        args = mock_post.call_args[0][1]
        assert "input_image" in args
...
class TestFlux2KleinImageSlots:
    ...
    def test_single_image(self, mock_post):
        ...
        img = make_image_tensor()
        run_async(Flux2Klein9bDirect.execute(prompt="test", safety_tolerance=2, output_format="jpeg", image_1=img))
        args = mock_post.call_args[0][1]
        assert "input_image" in args
        assert "input_image_2" not in args
    ...
    def test_all_four_images(self, mock_post):
        ...
        run_async(Flux2Klein9bDirect.execute(prompt="test", safety_tolerance=2, output_format="jpeg", **imgs))
        args = mock_post.call_args[0][1]
...
class TestOptionalArguments:
    ...
    def test_zero_width_height_excluded(self, mock_post):
        from nodes.flux2max_direct import Flux2MaxDirect
        run_async(Flux2MaxDirect.execute(prompt="test", safety_tolerance=2, output_format="jpeg", width=0, height=0))
        args = mock_post.call_args[0][1]
    ...
    def test_nonzero_width_height_included(self, mock_post):
        from nodes.flux2max_direct import Flux2MaxDirect
        run_async(Flux2MaxDirect.execute(prompt="test", safety_tolerance=2, output_format="jpeg", width=1024, height=768))
        args = mock_post.call_args[0][1]
    ...
    def test_default_seed_excluded(self, mock_post):
        from nodes.flux2max_direct import Flux2MaxDirect
        run_async(Flux2MaxDirect.execute(prompt="test", safety_tolerance=2, output_format="jpeg", seed=-1))
        args = mock_post.call_args[0][1]
    ...
    def test_explicit_seed_included(self, mock_post):
        from nodes.flux2max_direct import Flux2MaxDirect
        run_async(Flux2MaxDirect.execute(prompt="test", safety_tolerance=2, output_format="jpeg", seed=42))
        args = mock_post.call_args[0][1]
        assert args["seed"] == 42
    ...
    def test_empty_webhook_excluded(self, mock_post):
        from nodes.flux2max_direct import Flux2MaxDirect
        run_async(Flux2MaxDirect.execute(prompt="test", safety_tolerance=2, output_format="jpeg", webhook_url="", webhook_secret=""))
        args = mock_post.call_args[0][1]
...
class TestBlankImageFallback:
    ...
    def test_returns_blank_when_post_fails(self, mock_post):
        from nodes.flux2max_direct import Flux2MaxDirect
        result = run_async(Flux2MaxDirect.execute(prompt="test", safety_tolerance=2, output_format="jpeg"))
